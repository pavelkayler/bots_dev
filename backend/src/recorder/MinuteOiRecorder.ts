...
export class MinuteOiRecorder {
  ...
  private mode: MinuteOiRecorderMode = "record_while_running";
  ...
  ingestTicker(args: IngestArgs): boolean {
    ...
    if (!symbol || !Number.isFinite(oiv) || oiv <= 0 || !Number.isFinite(tsMs) || tsMs <= 0) return false;
