...
export class CvdRecorder {
  ...
  private mode: CvdRecorderMode = "record_while_running";
  ...
  private bucket1sPath(symbol: string, ts: number): string {
    return path.join(this.rootDir, "1s", symbol, `${toUtcDayKey(ts)}.jsonl`);
  }
  ...
  private flushSecondBucket(symbol: string, bucket: Cvd1sBucket) {
    writeJsonl(this.bucket1sPath(symbol, bucket.ts), bucket);
    this.writes1s += 1;
